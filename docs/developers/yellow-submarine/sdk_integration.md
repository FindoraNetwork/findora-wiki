...
```ts
  ...
  // we need to provide a commitment string which we would `transfer` to wallet B
  const commitment = 'YOUR_COMMITMENT';

  // `abarToBar` operation would require instances of abar, which will be created (restored)
