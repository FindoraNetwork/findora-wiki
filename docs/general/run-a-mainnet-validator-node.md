...
- `fs`: a command line tool for staking
...
> - Check that binaries have executable permissions set correctly
    >     - ex) `chmod +x tendermint abci_validator_node fs`
> - Check that binary files are placed into one of your `PATH` directories
    >     - ex) `mv tendermint abci_validator_node fs /usr/local/bin/`
...
```shell
fs genkey > ${LEDGER_DIR}/tmp.gen.keypair
```
...
```shell
fs setup -S https://prod-mainnet.prod.findora.org
...
#     fs setup -O ${LEDGER_DIR}/node.mnemonic
...
For staking operations, you should use the `fs` tool.
...
> ```
>
> Similar help information can be obtained through the `fs` tool itself:
>
> - `fs --help`
> - `fs stake --help`
> - `fs unstake --help`
...
fs stake -n $((1888888 * 1000000)) -R 0.2 -M 'Node-A'
```
